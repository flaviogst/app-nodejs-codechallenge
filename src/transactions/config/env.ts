...
const envFile = process.env.TRANSACTIONS_ENV_FILE ?? ".env";
...
const envSchema = z.object({
  ...
  KAFKA_CONSUMER_GROUP_TRANSACTIONS: z
    .string()
    .min(1, "KAFKA_CONSUMER_GROUP_TRANSACTIONS is required")
});
...
if (!parsed.success) {
  ...
  throw new Error(`Invalid transactions environment variables: ${formattedErrors}`);
}
